...
このアプリは逆に「何も起きていないこと」を知らせることで安心を提供します。
外部APIには一切依存せず、通信を行わない完全ローカル完結型のアプリです(プライバシー・費用ゼロ)。

ポートフォリオ・個人利用目的のアプリで、現時点ではApp Storeでの公開は想定していません。
...
| グラフ描画 | Swift Charts(統計画面の棒グラフ) |
| ホーム画面ウィジェット | WidgetKit(Small/Medium対応) |
| データ共有 | App Groups(アプリ本体⇔ウィジェット間の共有コンテナ) |
...
| S-07 | 統計画面 | 累計サマリーと直近30日のグラフで記録を俯瞰(カレンダー画面のツールバーから遷移) |

画面遷移: オンボーディング(初回のみ)→ ホーム/カレンダーのタブ切替、
ホームから追加画面へsheet遷移、カレンダーから日別詳細・統計画面へpush遷移、
通知タップで通知詳細画面へfullScreenCover遷移。

## ホーム画面ウィジェット

iOSのホーム画面にSmall/Medium 2サイズのウィジェットを配置できる(WidgetKit)。

| サイズ | 表示内容 |
|---|---|
| Small | 見守り中の件数を大きく表示(例:「2件、静かに見守り中」) |
| Medium | 見守り中の件数 + 最も経過時間が長い1件のタイトルと24時間進捗バー |

いずれもデータが0件のときは「穏やかです」の空状態表示に切り替わり、タップするとアプリ本体が起動する。
アプリ側でWorryItemを追加・削除・完了すると、`WidgetCenter.reloadAllTimelines()`により表示が即時に更新される。

## スクリーンショット

### アプリアイコン

三日月と星々をモチーフに、アプリ本体と同じカラーパレットでデザイン。

<img src="Screenshots/app-icon.png" alt="アプリアイコン" width="160">
...
| ホーム画面(S-01) | 追加画面(S-02) | カレンダー画面(S-04) | 統計画面(S-07) |
|---|---|---|---|
| ![home](Screenshots/home.png) | ![add-worry](Screenshots/add-worry.png) | ![calendar](Screenshots/calendar.png) | ![statistics](Screenshots/statistics.png) |
...
### 6. 別プロセス間(アプリ本体⇔ウィジェット)でのデータ共有設計
Widget ExtensionはアプリRoot本体とは別プロセスで動作するため、通常の`ModelContainer`では
互いのデータにアクセスできない。App Groupsで両ターゲットに同じグループIDを設定し、
SwiftDataの`ModelConfiguration`を`groupContainer: .identifier(...)`で初期化することで、
共有コンテナ上の同一データベースファイルを本体・ウィジェットの双方から読み書きできるようにした。
グループIDは`AppGroup.swift`という共有ファイルに定数化し、両ターゲットのTarget Membershipに
含めることで、本体側とウィジェット側でIDが食い違う事故を防いでいる。

### 7. #Predicate不使用の方針をウィジェット側にも一貫適用
工夫4で述べた「`#Predicate`を使わず全件取得+Swift側`filter`」という開発規約は、
ウィジェット側の`TimelineProvider`にもそのまま適用した。`WorryRepository`をウィジェット
ターゲットとも共有し、同じ`fetchActive()`を呼ぶことで、取得ロジックの二重実装を避けつつ
アプリ本体・ウィジェットの両方で不具合を回避する方式に統一している。

### 8. WidgetCenter.reloadAllTimelines()による即時反映
ウィジェットは通常iOSが決めたタイミングでしか更新されず、データ変更後もしばらく古い表示の
ままになりうる。この体験を防ぐため、`WorryRepository`のadd/delete/markAsCompletedの各メソッド内、
`context.save()`直後に`WidgetCenter.shared.reloadAllTimelines()`を呼び出す処理を追加した。
書き込み処理を1箇所(Repository)に集約している設計が活きており、呼び出し漏れの心配なく
全ての変更経路でウィジェットへの即時反映を実現している。
